...
<div class="container">
    ...
    <table class="table">
        <c:if test="${order.status.id != 2 && order.status.id !=3}">
            <tr>
                <td colspan="5" style="text-align: end">
                    <form action="${pageContext.request.contextPath}/api/orders/order_details/cancel?id=${order.id}"
                          method="post">
                        <button class="btn btn-outline-primary" type="submit"><fmt:message
                                key="orderDetails.cancelOrder"/></button>
                    </form>
                </td>
            </tr>
        </c:if>
